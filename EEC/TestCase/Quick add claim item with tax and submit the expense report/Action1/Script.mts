...
For i = 1 To .GetSheet("Entitlement").GetRowCount
	...
	If .Value(Initiate.GetCompanyCode, "Entitlement") = "T" or .Value(Initiate.GetCompanyCode, "Entitlement") = "V" Then
		...
		For j = 1 To .GetSheet("ExpenseData").GetRowCount
			...
			If .Value("ToTest", "ExpenseData") = "Y" or .Value("ToTest", "ExpenseData") = "T" Then
				...
				RunAction "Create Complete Claim (With Tax) [Quick Add]", oneIteration, strVal, bResult, strAmount, strTaxAmount
				...
				If bResult Then
					...
					arrValueToExcel = Array(Initiate.GetCompanyCode, "", "", Initiate.GetUsername, strRefNo, Datatable.Value("ExpenseCategory", "Entitlement"), strAmount, "Y", strTaxAmount)
					.GetSheet("Entitlement").AddParameter "Result" & j, Join(arrValueToExcel, ",")
				End If
