...
Datatable.ExportSheet TestResultDir & "\" & GetStringDate &"_Mapped_" & TestCaseName & ".xls", "TestListMapped", "MappedPayerCode"
Datatable.ExportSheet TestResultDir & "\" & GetStringDate &"_UnMapped_" & TestCaseName & ".xls", "TestListUnMapped", "UnMappedPayerCode"
...
For x = 1 To Datatable.GetSheet("TestListMapped").GetRowCount : Do
	...
	Dim strDefaultPayerDetails
	Browser("DKSH Connect").Page("Checkout").Sync
	
	'Validation #1: Payer Book pop-up
	If  Browser("DKSH Connect").Page("Checkout").WebElement("PayerBook").GetROProperty("Visible") = "False" And _
	 	Browser("DKSH Connect").Page("Checkout").WebElement("PayerDetails").Exist Then
			strDefaultPayerDetails = Trim(Browser("DKSH Connect").Page("Checkout").WebElement("PayerDetails").GetROProperty("innertext"))
	Else
		strDefaultPayerDetails = "False"
	End If 
	...
	If strPayerDetails = strDefaultPayerDetails Then
		bV2 = True
	ElseIf strDefaultPayerDetails = "False" Then
		'Logout	
		ConnectLogoutAndCloseBrowser
		...
		Datatable.Value("Result2", "TestListMapped") = "NA"
		Datatable.Value("Remarks", "TestListMapped") = "Payer Not Mapped"
		Exit Do
	Else
		bV2 = False
	End If
...
For y = 1 To Datatable.GetSheet("TestListUnMapped").GetRowCount : Do
	...
	'Search for product to add to cart @@ script infofile_;_ZIP::ssf8.xml_;_
	SearchProductAndAddToCart strProductCode, 1
	SetProductQuantityBasedOnMOV
	...
	'Validation #1: Payer Book pop-up
	If Browser("DKSH Connect").Page("Checkout").WebElement("PayerBook").GetROProperty("Visible") = "True" Then
		bV1 = True
	Else
		bV1 = False
		Browser("DKSH Connect").Page("Checkout").WebButton("Change payer").Click
	...
	If bV1 = False Then
		...
		Datatable.Value("Remarks", "TestListUnMapped") = "Payer Mapped"
	Else
		Datatable.Value("Result1", "TestListUnMapped") = bV1
		Datatable.Value("Result2", "TestListUnMapped") = bV2
		Datatable.Value("Result3", "TestListUnMapped") = bV3
